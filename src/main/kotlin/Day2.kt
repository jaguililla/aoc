 ...
 *
 * @param input Games data.
 * @param red Maximum red cubes.
 * @param green Maximum green cubes.
 * @param blue Maximum blue cubes.
 * @return Calculated result.
 */
fun day2Part1(input: String, red: Int, green: Int, blue: Int): Int {
    val gameSet = GameSet.parse(input)
    val cubeSet = CubeSet(red, green, blue)
    val gamesContaining = gameSet.gamesContaining(cubeSet)

    return gamesContaining.sumOf { it.id }
}

data class GameSet(val games: Set<Game>) {

    companion object {
        fun parse(input: String): GameSet =
            GameSet(input.lines().filter(String::isNotBlank).map(Game::parse).toSet())
    }

    fun gamesContaining(cubeSet: CubeSet): Set<Game> =
        games.filter { it.contains(cubeSet) }.toSet()
}

data class Game(val id: Int, val cubeSets: Set<CubeSet>) {

    companion object {
        fun parse(input: String): Game {
            val (k, v) = input.split(":")

            val id = k.lowercase().removePrefix("game").trim().toInt()
            val cubeSets = v.split(";").map(CubeSet::parse).toSet()

            return Game(id, cubeSets)
        }
    }

    val maximumCubeSet: CubeSet = CubeSet(
        cubeSets.maxOf(CubeSet::red),
        cubeSets.maxOf(CubeSet::green),
        cubeSets.maxOf(CubeSet::blue),
    )

    fun contains(cubeSet: CubeSet): Boolean =
        cubeSet.contains(maximumCubeSet)
}
...
data class CubeSet(val red: Int, val green: Int, val blue: Int) {
    ...
    companion object {
        fun parse(input: String): CubeSet {
            val cubes = input
                ...
                .split(",")
                .associate {
                    val (count, color) = it.trim().split(" ").filter(String::isNotEmpty)
                    color.lowercase() to count.toInt()
                }

            return CubeSet(cubes["red"] ?: 0, cubes["green"] ?: 0, cubes["blue"] ?: 0)
        }
    }
    ...
    fun contains(cubeSet: CubeSet): Boolean =
        (red >= cubeSet.red) && (green >= cubeSet.green) && (blue >= cubeSet.blue)
}
...
fun day2Part2(input: String): Int {
    val gameSet = GameSet.parse(input)
    return gameSet.games.sumOf { it.maximumCubeSet.power }
}
